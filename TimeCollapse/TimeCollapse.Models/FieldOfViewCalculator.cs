...
namespace TimeCollapse.Models
{
    public static class FieldOfViewCalculator
    {
        ...
        public static Point[] GetFieldOfViewNonTrivial(this Explorer e, Game game)
        {
            ...
            var nodesInSight =
                game.Map.Blocks
                    .Where(b => b.IntersectsWith(viewRect))
                    .SelectMany(b => new[]
                    {
                        new Vector(b.Left, b.Top) - start,
                        new Vector(b.Left, b.Bottom) - start,
                        new Vector(b.Right, b.Top) - start,
                        new Vector(b.Right, b.Top) - start
                    })
                    .SelectMany(v => new[]
                    {
            ...
            foreach (var node in nodesInSight)
            {
                ...
                {
                    current = node;
                    lastVectorCrossed = NearestCrossPoint(start, current, linesInSight, out point);
                    result.Add(point);
                }
                else
                {
                    while (Math.Atan2(current.Y, current.X) > Math.Atan2(node.Y, node.X))
                    {
                        current = current.Turn(deltaAngle);
                        result.Add(current.ToPoint());
                    }
        ...
        private static bool NearestCrossPoint(Vector start, Vector ray, IEnumerable<(Vector,Vector)> linesInSight, out Point point)
        {
            ...
            foreach (var line in linesInSight)
                if (RayCross(start, ray, line, out var cross))
                    if ((cross - start).Length < (shortestCrossedRay - start).Length)
                        shortestCrossedRay = cross;
            point = shortestCrossedRay.ToPoint();
            return !Equals(shortestCrossedRay, start + ray);
        }
